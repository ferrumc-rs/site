    ...
    <main className="flex-1">
      <article className="mx-auto max-w-4xl px-6 py-12">
        ...
        <header className="mb-12 border-b border-white/10 pb-8">
          ...
          {blog.description && (<p className="text-xl text-neutral-300 mt-6">
            {blog.description}
          </p>)}
        </header>
        ...
        <div className="markdown-content">
          <ReactMarkdown
                  components={{
                    h1: ({children}) => (<h1 className="text-4xl font-bold text-white mt-12 mb-6">
                      {children}
                    </h1>),
                    h2: ({children}) => (<h2 className="text-3xl font-bold text-white mt-12 mb-6">
                      {children}
                    </h2>),
                    h3: ({children}) => (<h3 className="text-2xl font-bold text-white mt-8 mb-4">
                      {children}
                    </h3>),
                    h4: ({children}) => (<h4 className="text-xl font-bold text-white mt-6 mb-3">
                      {children}
                    </h4>),
                    p: ({children}) => (<p className="text-neutral-300 text-lg leading-relaxed mb-6">
                      {children}
                    </p>),
                    ul: ({children}) => (<ul className="text-neutral-300 text-lg mb-6 ml-6 list-none space-y-3">
                      {children}
                    </ul>),
                    ol: ({children}) => (<ol className="text-neutral-300 text-lg mb-6 ml-6 list-decimal space-y-3">
                      {children}
                    </ol>),
                    li: ({children}) => (<li className="text-neutral-300 flex items-start">
                    ...
                    code: ({children, className}) => {
                      ...
                      return isInline ? (<code className="text-orange-400 bg-white/5 px-1.5 py-0.5 rounded text-sm">
                        {children}
                    ...
                    blockquote: ({children}) => (<blockquote
                            className="border-l-4 border-orange-500 pl-4 my-6 text-neutral-400 italic">
                      {children}
                    </blockquote>),
                    img: ({src, alt}) => (<img
                            src={src}
