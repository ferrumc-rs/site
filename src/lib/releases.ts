...
export interface ReleaseAsset {
  platform: 'docker' | 'windows-x64' | 'windows-arm64' | 'linux-x64' | 'linux-arm64' | 'macos-x64' | 'macos-arm64';
  url: string;
  filename: string;
  size: number;
}
...
function detectPlatform(filename: string): ReleaseAsset['platform'] | null {
  ...
  if (lower.includes('windows') && lower.includes('x86_64')) return 'windows-x64';
  if (lower.includes('windows') && lower.includes('aarch64')) return 'windows-arm64';
  if (lower.includes('linux') && lower.includes('x86_64')) return 'linux-x64';
  if (lower.includes('linux') && lower.includes('aarch64')) return 'linux-arm64';
  if (lower.includes('darwin') && lower.includes('x86_64')) return 'macos-x64';
  if (lower.includes('darwin') && lower.includes('aarch64')) return 'macos-arm64';
  if (lower.includes('apple') && lower.includes('x86_64')) return 'macos-x64';
  if (lower.includes('apple') && lower.includes('aarch64')) return 'macos-arm64';
